...
function materialize_traced_array(x::LinearAlgebra.Transpose{<:Any,<:TracedRArray})
    px = parent(x)
    A = ndims(px) == 1 ? reshape(px, :, 1) : px
    return permutedims(A, (2, 1))
end
function materialize_traced_array(x::LinearAlgebra.Adjoint{<:Any,<:TracedRArray})
