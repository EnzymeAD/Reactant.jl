...
function TracedUtils.materialize_traced_array(x::Transpose{T,TracedRArray{T,N}}) where {T,N}
    px = parent(x)
    A = ndims(px) == 1 ? reshape(px, :, 1) : px
    return permutedims(A, (2, 1))
end

function TracedUtils.materialize_traced_array(x::Adjoint{T,TracedRArray{T,N}}) where {T,N}
    return conj(materialize_traced_array(transpose(parent(x))))
end

function TracedUtils.materialize_traced_array(x::Diagonal{T,TracedRArray{T,1}}) where {T}
...
function TracedUtils.set_mlir_data!(x::Transpose{T,TracedRArray{T,N}}, data) where {T,N}
...
function TracedUtils.set_mlir_data!(x::Adjoint{T,TracedRArray{T,N}}, data) where {T,N}
    tdata = TracedRArray{T}(data)
    px = parent(x)
    transposed_data =
...
function TracedUtils.set_mlir_data!(x::Diagonal{T,TracedRArray{T,1}}, data) where {T}
