...
function MakeMetalClient(;
    ...
    if metal_pjrt_plugin_path !== nothing
        ...
        return MakeMetalClientFromApi(_metal_pjrt_api_ptr[])
    end
end
...
function MakeMetalClientFromApi(api_ptr::Ptr{Cvoid})
    errstr = Ref{Cstring}()
...
# Registered by ReactantMetalExt when Metal.jl is loaded (Option A: direct PJRT_Api registration)
