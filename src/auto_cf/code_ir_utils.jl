...
function change_stmt!(ir::Core.Compiler.IRCode, ssa::Int, stmt, return_type::Type)
    return Core.Compiler.inst_from_newinst!(
        ir[Core.SSAValue(ssa)], Core.Compiler.NewInstruction(stmt, return_type), Int32(0), UInt32(0)
    )
end
...
"""
function change_stmt!(ir::Core.Compiler.IRCode, ssa::Int, goto::Core.GotoNode, return_type::Type)
...
function clear_block_ir!(ir::Core.Compiler.IRCode, blocks::Set{Int})
    for block in blocks
        ...
        (f, l) = (stmt_range |> first, stmt_range |> last)
        for i in f:l
...
"""
    type_from_ssa(ir::Core.Compiler.IRCode, args::Dict, v::Vector)::Vector
    For each stmt in `v` in `ir` get its type 
"""
function type_from_ssa(ir::Core.Compiler.IRCode, args::Dict, v)
    [
        begin
...
function apply_map(array, block_map)
    [block_map[a] for a in array if haskey(block_map, a)]
end
...
function new_cfg(ir, to_extract, block_map)
    ...
    for b in to_extract
        ...
        (; start, stop) = bb.stmts
        diff = stop - start
        push!(bbs, Core.Compiler.BasicBlock(Core.Compiler.StmtRange(n, diff + n),
            apply_map(bb.preds, block_map),
            apply_map(bb.succs, block_map)))
        n += diff + 1
        push!(index, n)
    end
    Core.Compiler.CFG(bbs, index)
end
...
function is_a_terminator(stmt)
    stmt isa Union{Core.GotoNode,Core.ReturnNode,Core.GotoIfNot}
end
...
function offset_stmt!(dict::Dict, stmt, offset::Dict, ir::Core.Compiler.IRCode, bb_map)
    if stmt isa Expr
        Expr(
            stmt.head, (offset_stmt!(dict, a, offset, ir, bb_map) for a in stmt.args)...)
    ...
    elseif stmt isa Core.GotoIfNot
        Core.GotoIfNot(offset_stmt!(dict, stmt.cond, offset, ir, bb_map), get(bb_map, stmt.dest, 0))
    elseif stmt isa Core.PhiNode
        Core.PhiNode(Int32[bb_map[edge] for edge in stmt.edges], Any[offset_stmt!(dict, value, offset, ir, bb_map) for value in stmt.values])
...
"""
function extract_multiple_block_ir(ir::Core.Compiler.IRCode, to_extract_set::Set{Int}, args::Dict, new_returns::Vector)::WipExtracting
    ...
    cumulative_offset = (ir.cfg.blocks[first(to_extract)].stmts |> first) - 1
    new_n_stmt = 0
    for bb in minimum(to_extract):maximum(to_extract)
        n_stmt = ir.cfg.blocks[bb].stmts |> length
        if bb in to_extract
    ...
    f = ir.cfg.blocks[first(to_extract)].stmts |> first
    l = ir.cfg.blocks[last(to_extract)].stmts |> last
    ...
    for bb in to_extract
        ...
        for old_stmt in range_bb[1]:range_bb[2]
            ...
            #ssa offset
            instruction_stream.stmt[new_stmt] = offset_stmt!(args, ir.stmts.stmt[old_stmt], bb_offset, ir, block_map)
            #line_info
    ...
    linetable = [Core.LineInfoNode(l.module, l.method, l.file, l.line, Int32(0)) for l in linetable]
    #Build the new IR argtypes from args dictionnary
    (_, argtypes) = vec_args(ir, args)
    new_ir = Core.Compiler.IRCode(instruction_stream, cfg, linetable, argtypes, Expr[], Core.Compiler.VarState[])

    #JuliaIR block can end without a terminator
    has_terminator = is_a_terminator(instruction_stream.stmt[end])

    n_ssa = length(instruction_stream)
    retu = if length(new_returns) > 1
        tuple = Core.Compiler.NewInstruction(Expr(:call, Core.GlobalRef(Core, :tuple), new_returns...), Tuple{type_from_ssa(new_ir, args, new_returns)...})
        Core.Compiler.insert_node!(new_ir, Core.Compiler.SSAValue(n_ssa), tuple, !has_terminator)
    else
    ...
    WipExtracting(Core.Compiler.compact!(new_ir, true))
end

function mlir_type(x)
    return Reactant.MLIR.IR.TensorType(size(x), Reactant.MLIR.IR.Type(Reactant.unwrapped_eltype(x)))
end
...
"""
function add_phi_value!(v::Vector, phi::Core.PhiNode, edge::Set{Int})
...
function cond_ssa(ir::CC.IRCode, bb::Int)
    ...
    terminator isa Core.GotoIfNot || return
    protection = ir.stmts.stmt[terminator.cond.id]
    (protection isa Expr && protection.head == :call && protection.args[1] == Core.GlobalRef(@__MODULE__, :traced_protection)) || return
    protection.args[2]
end
...
function check_integrity(ir::CC.IRCode)::Bool
    !any(ir.stmts.stmt .== [Core.ReturnNode()])
end