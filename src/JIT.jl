...
MethodInstanceKey = Tuple{Vector{Type}}
...
function CC.optimize(
    interp::ReactantInterpreter, opt::CC.OptimizationState, caller::CC.InferenceResult
)
    tree = get(interp.meta_data[].traced_tree_map, mi_key(opt.linfo), nothing)
    CC.@timeit "optimizer" ir = if isnothing(tree) || isempty(tree)
        CC.run_passes_ipo_safe(opt.src, opt, caller)
    else
        run_passes_ipo_safe_auto_cf(opt.src, opt, caller, tree)
    end
...
fake_oc(ir::CC.IRCode, return_type) = begin
    src = ir_to_codeinfo!(ir)
    fake_oc(src, return_type)
end

function fake_oc(src::CC.CodeInfo, return_type; args=nothing)
