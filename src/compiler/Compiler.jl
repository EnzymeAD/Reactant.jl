...
Base.@nospecializeinfer function compile_mlir!(
    ...
    enzyme_pass = enzyme_pass_pipeline(;
        jit_backend=if backend in ("cpu", "tpu", "neuron", "trainium")
            "cpu"
        else
            "cuda"
        end
