...
function with_profiler(
    ...
    profiler = @ccall Reactant.MLIR.API.CreateProfilerSession(
    ...
    results = try
        ...
        @ccall Reactant.MLIR.API.ProfilerSessionCollectData(
            profiler::Ptr{Cvoid}, trace_output_dir::Cstring
        )::Cvoid
        @ccall Reactant.MLIR.API.ProfilerSessionDelete(profiler::Ptr{Cvoid})::Cvoid
    end
...
function profiler_activity_start(name::String, level::Cint)
    return @ccall Reactant.MLIR.API.ProfilerActivityStart(name::Cstring, level::Cint)::Int64
end
...
function profiler_activity_end(id::Int64)
    return @ccall Reactant.MLIR.API.ProfilerActivityEnd(id::Int64)::Cvoid
end
...
@inline function free_profiler(exec)
    @ccall Reactant.MLIR.API.ProfilerServerStop(exec.exec::Ptr{Cvoid})::Cvoid
end
...
mutable struct ProfileServer
    ...
    function ProfileServer(port)
        exec = @ccall Reactant.MLIR.API.ProfilerServerStart(port::Int32)::Ptr{Cvoid}
        @assert exec != C_NULL
...
function initialize_xprof_stubs(worker_service_address::String)
    @ccall Reactant.MLIR.API.InitializeXProfStubs(worker_service_address::Cstring)::Cvoid
    return nothing
end
...
function start_xprof_grpc_server(port::Integer)
    @ccall Reactant.MLIR.API.StartGrpcServer(port::Cint)::Cvoid
    return nothing
end
...
function xspace_to_tools_data(
    ...
    ret = @ccall Reactant.MLIR.API.XSpaceToToolsData(
