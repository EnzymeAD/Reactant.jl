...
function DenseElementsAttribute(values::AbstractArray{T}) where {T<:Complex}
    shaped_type = TensorType(size(values), Type(T))
    # TODO: row major
    return Attribute(
        API.mlirDenseElementsAttrRawBufferGet(
            shaped_type, length(values) * sizeof(T), values
        ),
    )
end
