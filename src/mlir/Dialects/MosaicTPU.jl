...
function all_reduce(input::Value; output=nothing::Union{Nothing, IR.Type}, dim, kind, location=Location())
...
function assume_multiple(value::Value; result=nothing::Union{Nothing, IR.Type}, multiple, location=Location())
    op_ty_results = IR.Type[]
...
function concatenate(sources::Vector{Value}; output::IR.Type, dimension, location=Location())
    op_ty_results = IR.Type[output, ]
...
"""
function create_subelement_mask(; output::IR.Type, from, to, num_subelems, location=Location())
    op_ty_results = IR.Type[output, ]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[namedattribute("from", from), namedattribute("to", to), namedattribute("num_subelems", num_subelems), ]
...
function dynamic_gather(source::Value, indices::Value; output::IR.Type, dimension, location=Location())
    op_ty_results = IR.Type[output, ]
...
function dynamic_rotate(value::Value, amount::Value; result::IR.Type, dimension, stride=nothing, stride_dimension=nothing, location=Location())
    op_ty_results = IR.Type[result, ]
...
function enqueue_dma(source::Value, source_semaphore=nothing::Union{Nothing, Value}; target::Value, target_semaphore::Value, device_id=nothing::Union{Nothing, Value}, core_id=nothing::Union{Nothing, Value}, location=Location())
    op_ty_results = IR.Type[]
...
function load(base::Value, indices::Vector{Value}; result::IR.Type, sublane_mask, sublane_stride=nothing, location=Location())
    op_ty_results = IR.Type[result, ]
...
function matmul(lhs::Value, rhs::Value, acc::Value; result::IR.Type, transpose_lhs=nothing, transpose_rhs=nothing, precision=nothing, dimension_numbers=nothing, location=Location())
    op_ty_results = IR.Type[result, ]
...
function pack_subelements(sources::Vector{Value}; output::IR.Type, positions, pack_format, location=Location())
    op_ty_results = IR.Type[output, ]
    operands = Value[sources..., ]
    owned_regions = Region[]
    successors = Block[]
...
function rotate(value::Value; result=nothing::Union{Nothing, IR.Type}, amount, dimension, stride=nothing, stride_dimension=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[value, ]
    owned_regions = Region[]
    successors = Block[]
...
function sem_read(semaphore::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
...
function sem_signal(semaphore::Value, amount::Value, device_id=nothing::Union{Nothing, Value}; core_id=nothing::Union{Nothing, Value}, core_type=nothing, location=Location())
    op_ty_results = IR.Type[]
...
function shuffled_load(base::Value, indices::Vector{Value}; result::IR.Type, sublane_mask, sublane_offsets, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[base, indices..., ]
    owned_regions = Region[]
    successors = Block[]
...
function shuffled_store(valueToStore::Value, base::Value, indices::Vector{Value}; sublane_mask, sublane_offsets, location=Location())
    op_ty_results = IR.Type[]
...
function store(valueToStore::Value, base::Value, indices::Vector{Value}, mask=nothing::Union{Nothing, Value}; sublane_mask, sublane_stride=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[valueToStore, base, indices..., ]
    owned_regions = Region[]
    successors = Block[]
...
function strided_load(base::Value, indices::Vector{Value}; result::IR.Type, strides, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[base, indices..., ]
    owned_regions = Region[]
    successors = Block[]
...
function strided_store(valueToStore::Value, base::Value, indices::Vector{Value}; strides, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[valueToStore, base, indices..., ]
    owned_regions = Region[]
    successors = Block[]
...
function trace(; results::Vector{IR.Type}, message, level, region::Region, location=Location())
    op_ty_results = IR.Type[results..., ]
    operands = Value[]
    owned_regions = Region[region, ]
    successors = Block[]
...
function unpack_subelements(source::Value; output::IR.Type, index, pack_format, location=Location())
    op_ty_results = IR.Type[output, ]
    operands = Value[source, ]
    owned_regions = Region[]
    successors = Block[]
...
function vector_store(valueToStore::Value, base::Value, indices::Vector{Value}, mask=nothing::Union{Nothing, Value}; strides, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[valueToStore, base, indices..., ]
    owned_regions = Region[]
    successors = Block[]
