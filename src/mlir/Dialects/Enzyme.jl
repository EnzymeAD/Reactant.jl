...
function autodiff(inputs::Vector{Value}; outputs::Tuple{Vararg{IR.Type}}, fn::Attribute, activity::Attribute, ret_activity::Attribute, width::Union{Int64, Nothing}=nothing, location=Location())
...
function batch(inputs::Vector{Value}; outputs::Tuple{Vararg{IR.Type}}, fn::Attribute, batch_shape::Vector{Int64}, location=Location())
    op_ty_results = IR.Type[outputs..., ]
    operands = Value[inputs..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[namedattribute("fn", fn), namedattribute("batch_shape", Attribute(batch_shape)), ]
...
function fwddiff(inputs::Vector{Value}; outputs::Tuple{Vararg{IR.Type}}, fn::Attribute, activity::Attribute, ret_activity::Attribute, width::Union{Int64, Nothing}=nothing, location=Location())
    op_ty_results = IR.Type[outputs..., ]
    operands = Value[inputs..., ]
    owned_regions = Region[]
    successors = Block[]
...
function genericAdjoint(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Tuple{Vararg{IR.Type}}, indexing_maps::Attribute, iterator_types::Attribute, doc::Union{String, Nothing}=nothing, library_call::Union{String, Nothing}=nothing, region::Region, location=Location())
    ...
    !isnothing(library_call) && push!(attributes, namedattribute("library_call", Attribute(library_call)))
