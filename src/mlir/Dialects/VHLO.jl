...
function batch_norm_grad_v1(
    ...
    grad_scale::IR.Type,
    grad_offset::IR.Type,
    epsilon::IR.Attribute,
    feature_index::IR.Attribute,
    location::Location=Location(),
)
...
function broadcast_in_dim_v1(
    operand::Value;
    result::IR.Type,
    broadcast_dimensions::IR.Attribute,
    location::Location=Location(),
)
    op_ty_results = IR.Type[result,]
    operands = Value[operand,]
...
function cholesky_v1(
    a::Value; result::IR.Type, lower::IR.Attribute, location::Location=Location()
)
    op_ty_results = IR.Type[result,]
    operands = Value[a,]
    owned_regions = Region[]
...
function collective_permute_v1(
    operand::Value;
    result::IR.Type,
    source_target_pairs::IR.Attribute,
    channel_id::IR.Attribute,
    location::Location=Location(),
)
    op_ty_results = IR.Type[result,]
...
function compare_v1(
    ...
    rhs::Value;
    result::IR.Type,
    comparison_direction::IR.Attribute,
    compare_type::IR.Attribute,
    location::Location=Location(),
)
    op_ty_results = IR.Type[result,]
    operands = Value[lhs, rhs]
...
function convolution_v1(
    ...
    padding::IR.Attribute,
    lhs_dilation::IR.Attribute,
    rhs_dilation::IR.Attribute,
    window_reversal::IR.Attribute,
    input_batch_dimension::IR.Attribute,
    input_feature_dimension::IR.Attribute,
    input_spatial_dimensions::IR.Attribute,
    kernel_input_feature_dimension::IR.Attribute,
    kernel_output_feature_dimension::IR.Attribute,
    kernel_spatial_dimensions::IR.Attribute,
    output_batch_dimension::IR.Attribute,
    output_feature_dimension::IR.Attribute,
    output_spatial_dimensions::IR.Attribute,
    feature_group_count::IR.Attribute,
    batch_group_count::IR.Attribute,
    precision_config::IR.Attribute,
    location::Location=Location(),
)
    op_ty_results = IR.Type[result,]
    operands = Value[lhs, rhs]
...
function custom_call_v1(
    ...
    api_version::IR.Attribute,
    called_computations::IR.Attribute,
    operand_layouts::IR.Attribute,
    result_layouts::IR.Attribute,
...
function dot_general_v1(
    ...
    rhs::Value;
    result::IR.Type,
    lhs_batching_dimensions::IR.Attribute,
    rhs_batching_dimensions::IR.Attribute,
    lhs_contracting_dimensions::IR.Attribute,
    rhs_contracting_dimensions::IR.Attribute,
    precision_config::IR.Attribute,
    location::Location=Location(),
)
    op_ty_results = IR.Type[result,]
...
function dot_general_v2(
    ...
    rhs_contracting_dimensions::IR.Attribute,
    precision_config::IR.Attribute,
    lhs_precision_type::IR.Attribute,
    rhs_precision_type::IR.Attribute,
    accumulation_type::IR.Attribute,
    lhs_component_count::IR.Attribute,
    rhs_component_count::IR.Attribute,
    num_primitive_operations::IR.Attribute,
    allow_imprecise_accumulation::IR.Attribute,
    location::Location=Location(),
)
    op_ty_results = IR.Type[result,]
    operands = Value[lhs, rhs]
...
function dynamic_broadcast_in_dim_v1(
    ...
    result::IR.Type,
    broadcast_dimensions::IR.Attribute,
    known_expanding_dimensions::IR.Attribute,
    known_nonexpanding_dimensions::IR.Attribute,
    location::Location=Location(),
)
    op_ty_results = IR.Type[result,]
...
function dynamic_gather_v1(
    ...
    slice_sizes::Value;
    result::IR.Type,
    offset_dims::IR.Attribute,
    collapsed_slice_dims::IR.Attribute,
    start_index_map::IR.Attribute,
    index_vector_dim::IR.Attribute,
    indices_are_sorted::IR.Attribute,
    location::Location=Location(),
)
    op_ty_results = IR.Type[result,]
...
function dynamic_gather_v2(
    ...
    offset_dims::IR.Attribute,
    collapsed_slice_dims::IR.Attribute,
    operand_batching_dims::IR.Attribute,
    start_indices_batching_dims::IR.Attribute,
    start_index_map::IR.Attribute,
    index_vector_dim::IR.Attribute,
    indices_are_sorted::IR.Attribute,
    location::Location=Location(),
...
function fft_v1(
    operand::Value;
    result::IR.Type,
    fft_type::IR.Attribute,
    fft_length::IR.Attribute,
    location::Location=Location(),
)
    op_ty_results = IR.Type[result,]
    operands = Value[operand,]
...
function func_v1(;
    sym_name::IR.Attribute,
    function_type::IR.Attribute,
    sym_visibility::IR.Attribute,
    arg_attrs::IR.Attribute,
    res_attrs::IR.Attribute,
    body::Region,
    location::Location=Location(),
)
...
function get_dimension_size_v1(
    operand::Value; result::IR.Type, dimension::IR.Attribute, location::Location=Location()
)
    op_ty_results = IR.Type[result,]
    operands = Value[operand,]
    owned_regions = Region[]
...
function get_tuple_element_v1(
    operand::Value; result::IR.Type, index::IR.Attribute, location::Location=Location()
)
    op_ty_results = IR.Type[result,]
    operands = Value[operand,]
    owned_regions = Region[]
...
function iota_v1(;
    output::IR.Type, iota_dimension::IR.Attribute, location::Location=Location()
)
    op_ty_results = IR.Type[output,]
    operands = Value[]
    owned_regions = Region[]
...
function pad_v1(
    ...
    padding_value::Value;
    result::IR.Type,
    edge_padding_low::IR.Attribute,
    edge_padding_high::IR.Attribute,
    interior_padding::IR.Attribute,
    location::Location=Location(),
)
    op_ty_results = IR.Type[result,]
...
function reduce_precision_v1(
    operand::Value;
    output::IR.Type,
    exponent_bits::IR.Attribute,
    mantissa_bits::IR.Attribute,
    location::Location=Location(),
)
    op_ty_results = IR.Type[output,]
    operands = Value[operand,]
...
function reduce_scatter_v1(
    operand::Value;
    result::IR.Type,
    scatter_dimension::IR.Attribute,
    replica_groups::IR.Attribute,
    channel_id::IR.Attribute,
    use_global_device_ids::IR.Attribute,
    computation::Region,
...
function reverse_v1(
    operand::Value; result::IR.Type, dimensions::IR.Attribute, location::Location=Location()
)
    op_ty_results = IR.Type[result,]
    operands = Value[operand,]
    owned_regions = Region[]
...
function scatter_v2(
    ...
    scatter_indices_batching_dims::IR.Attribute,
    scatter_dims_to_operand_dims::IR.Attribute,
    index_vector_dim::IR.Attribute,
    indices_are_sorted::IR.Attribute,
    unique_indices::IR.Attribute,
...
function slice_v1(
    operand::Value;
    result::IR.Type,
    start_indices::IR.Attribute,
    limit_indices::IR.Attribute,
    strides::IR.Attribute,
    location::Location=Location(),
)
    op_ty_results = IR.Type[result,]
    operands = Value[operand,]
...
function triangular_solve_v1(
    ...
    b::Value;
    result::IR.Type,
    left_side::IR.Attribute,
    lower::IR.Attribute,
    unit_diagonal::IR.Attribute,
    transpose_a::IR.Attribute,
    location::Location=Location(),
)
    op_ty_results = IR.Type[result,]
    operands = Value[a, b]
