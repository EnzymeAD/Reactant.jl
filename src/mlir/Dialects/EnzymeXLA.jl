...
function scope(operands::Vector{Value}; results::Vector{IR.Type}, region::Region, location=Location())
...
function alternatives(; regions::Vector{Region}, location=Location())
    ...
    create_operation(
        "enzymexla.alternatives", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function comm_region(; result_0::Vector{IR.Type}, body::Region, location=Location())
    ...
    create_operation(
        "enzymexla.comm_region", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function extend(operand::Value; result=nothing::Union{Nothing, IR.Type}, lhs, rhs, dimension, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[namedattribute("lhs", lhs), namedattribute("rhs", rhs), namedattribute("dimension", dimension), ]
    !isnothing(result) && push!(op_ty_results, result)
    
    create_operation(
        "enzymexla.extend", location;
        operands, owned_regions, successors, attributes,
...
function gpu_block(blockIndexX::Value, blockIndexY::Value, blockIndexZ::Value; region::Region, location=Location())
    op_ty_results = IR.Type[]
...
function gpu_kernel_address(; result::IR.Type, fn, location=Location())
    ...
    create_operation(
        "enzymexla.gpu_kernel_address", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function gpu_occupancy(blockSize::Value, dynamicSMemSize::Value, flags::Value; result::IR.Type, fn, location=Location())
    op_ty_results = IR.Type[result, ]
    ...
    create_operation(
        "enzymexla.gpu_occupancy", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function gpu_thread(threadIndexX::Value, threadIndexY::Value, threadIndexZ::Value; region::Region, location=Location())
    op_ty_results = IR.Type[]
...
params
"""
function gpu_wrapper(blockDims::Vector{Value}; result::IR.Type, region::Region, location=Location())
    op_ty_results = IR.Type[result, ]
    ...
    create_operation(
        "enzymexla.gpu_wrapper", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function lapack_gemqrt(V::Value, T::Value, C::Value; output::IR.Type, side, transpose=nothing, location=Location())
    ...
    create_operation(
        "enzymexla.lapack.gemqrt", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function lapack_geqrf(input::Value; output::IR.Type, tau::IR.Type, info::IR.Type, location=Location())
    ...
    create_operation(
        "enzymexla.lapack.geqrf", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function lapack_geqrt(input::Value; output::IR.Type, T::IR.Type, info::IR.Type, blocksize=nothing, location=Location())
    ...
    create_operation(
        "enzymexla.lapack.geqrt", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function jit_call(inputs::Vector{Value}; result_0::Vector{IR.Type}, fn, backend_config=nothing, operand_layouts=nothing, result_layouts=nothing, arg_attrs=nothing, res_attrs=nothing, output_operand_aliases=nothing, xla_side_effect_free=nothing, location=Location())
    op_ty_results = IR.Type[result_0..., ]
    ...
    !isnothing(xla_side_effect_free) && push!(attributes, namedattribute("xla_side_effect_free", xla_side_effect_free))
    
    create_operation(
        "enzymexla.jit_call", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function kernel_call(gridx::Value, gridy::Value, gridz::Value, blockx::Value, blocky::Value, blockz::Value, shmem::Value, inputs::Vector{Value}; result_0::Vector{IR.Type}, fn, backend_config=nothing, operand_layouts=nothing, result_layouts=nothing, arg_attrs=nothing, res_attrs=nothing, output_operand_aliases=nothing, xla_side_effect_free=nothing, location=Location())
    ...
    create_operation(
        "enzymexla.kernel_call", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function linalg_lu(input::Value; output::IR.Type, pivots::IR.Type, permutation::IR.Type, info::IR.Type, location=Location())
...
function memref2pointer(source::Value; result::IR.Type, location=Location())
    ...
    create_operation(
        "enzymexla.memref2pointer", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function lapack_orgqr(input::Value, tau::Value; output::IR.Type, location=Location())
    ...
    create_operation(
        "enzymexla.lapack.orgqr", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function lapack_ormqr(A::Value, tau::Value, C::Value; output::IR.Type, side, transpose=nothing, location=Location())
    op_ty_results = IR.Type[output, ]
    ...
    create_operation(
        "enzymexla.lapack.ormqr", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function pointer2memref(source::Value; result::IR.Type, location=Location())
    ...
    create_operation(
        "enzymexla.pointer2memref", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function polygeist_yield(; location=Location())
    ...
    create_operation(
        "enzymexla.polygeist_yield", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function rotate(operand::Value; result=nothing::Union{Nothing, IR.Type}, amount, dimension, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
function linalg_svd(input::Value; U::IR.Type, S::IR.Type, Vt::IR.Type, info::IR.Type, full=nothing, location=Location())
...
function stream2token(source::Value; result::IR.Type, location=Location())
    ...
    create_operation(
        "enzymexla.stream2token", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function lapack_symm(A::Value, B::Value, C::Value, alpha::Value, beta::Value; output::IR.Type, side, uplo, location=Location())
    op_ty_results = IR.Type[output, ]
    ...
    create_operation(
        "enzymexla.lapack.symm", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function wrap(operand::Value; result=nothing::Union{Nothing, IR.Type}, lhs, rhs, dimension, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
function xla_wrapper(inputs::Vector{Value}; fn, arg_attrs=nothing, res_attrs=nothing, location=Location())
    ...
    create_operation(
        "enzymexla.xla_wrapper", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
