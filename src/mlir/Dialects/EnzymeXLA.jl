...
function kernel_call(gridx::Value, gridy::Value, gridz::Value, blockx::Value, blocky::Value, blockz::Value, shmem::Value, inputs::Vector{Value}; result::Tuple{Vararg{IR.Type}}, fn::Attribute, backend_config::Union{String, Nothing}=nothing, operand_layouts::Union{Attribute, Nothing}=nothing, result_layouts::Union{Attribute, Nothing}=nothing, output_operand_aliases::Union{Attribute, Nothing}=nothing, location=Location())
    ...
    !isnothing(backend_config) && push!(attributes, namedattribute("backend_config", Attribute(backend_config)))
