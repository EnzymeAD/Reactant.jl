...
```
"""
function call_indirect(callee::Value, callee_operands::Vector{Value}; results::Tuple{Vararg{IR.Type}}, location=Location())
...
```
"""
function call(operands::Vector{Value}; result::Tuple{Vararg{IR.Type}}, callee::Attribute, no_inline::Union{Attribute, Nothing}=nothing, location=Location())
...
```
"""
function func_(; sym_name::Attribute, function_type::Attribute, sym_visibility::Union{String, Nothing}=nothing, arg_attrs::Union{Attribute, Nothing}=nothing, res_attrs::Union{Attribute, Nothing}=nothing, no_inline::Union{Attribute, Nothing}=nothing, body::Region, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[body, ]
    successors = Block[]
    attributes = NamedAttribute[namedattribute("sym_name", sym_name), namedattribute("function_type", function_type), ]
    !isnothing(sym_visibility) && push!(attributes, namedattribute("sym_visibility", Attribute(sym_visibility)))
