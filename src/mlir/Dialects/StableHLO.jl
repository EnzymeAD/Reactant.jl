...
```
"""
function all_gather(operands::Vector{Value}; result::Tuple{Vararg{IR.Type}}, all_gather_dim::Int64, replica_groups::Attribute, channel_handle::Union{Attribute, Nothing}=nothing, use_global_device_ids::Union{Attribute, Nothing}=nothing, location=Location())
...
```
"""
function all_reduce(operands::Vector{Value}; result::Tuple{Vararg{IR.Type}}, replica_groups::Attribute, channel_handle::Union{Attribute, Nothing}=nothing, use_global_device_ids::Union{Attribute, Nothing}=nothing, computation::Region, location=Location())
    op_ty_results = IR.Type[result..., ]
    operands = Value[operands..., ]
...
```
"""
function all_to_all(operands::Vector{Value}; result::Union{Nothing, Tuple{Vararg{IR.Type}}}=nothing, split_dimension::Int64, concat_dimension::Int64, split_count::Int64, replica_groups::Attribute, channel_handle::Union{Attribute, Nothing}=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operands..., ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function collective_broadcast(operand::Value; result::Union{Nothing, IR.Type}=nothing, replica_groups::Attribute, channel_handle::Union{Attribute, Nothing}=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function collective_permute(operand::Value; result::Union{Nothing, IR.Type}=nothing, source_target_pairs::Attribute, channel_handle::Union{Attribute, Nothing}=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function convolution(lhs::Value, rhs::Value; result::IR.Type, window_strides::Union{Vector{Int64}, Nothing}=nothing, padding::Union{Attribute, Nothing}=nothing, lhs_dilation::Union{Vector{Int64}, Nothing}=nothing, rhs_dilation::Union{Vector{Int64}, Nothing}=nothing, window_reversal::Union{Attribute, Nothing}=nothing, dimension_numbers::Attribute, feature_group_count::Int64, batch_group_count::Int64, precision_config::Union{Attribute, Nothing}=nothing, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[lhs, rhs, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[namedattribute("dimension_numbers", dimension_numbers), namedattribute("feature_group_count", Attribute(feature_group_count)), namedattribute("batch_group_count", Attribute(batch_group_count)), ]
...
```
"""
function dot_general(lhs::Value, rhs::Value; result::IR.Type, dot_dimension_numbers::Attribute, precision_config::Union{Attribute, Nothing}=nothing, algorithm::Union{Attribute, Nothing}=nothing, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[lhs, rhs, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function dynamic_conv(lhs::Value, rhs::Value, padding::Value; result::IR.Type, window_strides::Union{Vector{Int64}, Nothing}=nothing, lhs_dilation::Union{Vector{Int64}, Nothing}=nothing, rhs_dilation::Union{Vector{Int64}, Nothing}=nothing, window_reversal::Union{Attribute, Nothing}=nothing, dimension_numbers::Attribute, feature_group_count::Int64, batch_group_count::Int64, precision_config::Union{Attribute, Nothing}=nothing, location=Location())
    op_ty_results = IR.Type[result, ]
...
```
"""
function dynamic_gather(operand::Value, start_indices::Value, slice_sizes::Value; result::Union{Nothing, IR.Type}=nothing, dimension_numbers::Attribute, indices_are_sorted::Union{Bool, Nothing}=nothing, location=Location())
    op_ty_results = IR.Type[]
...
```
"""
function gather(operand::Value, start_indices::Value; result::Union{Nothing, IR.Type}=nothing, dimension_numbers::Attribute, slice_sizes::Vector{Int64}, indices_are_sorted::Union{Bool, Nothing}=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, start_indices, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[namedattribute("dimension_numbers", dimension_numbers), namedattribute("slice_sizes", Attribute(slice_sizes)), ]
    !isnothing(result) && push!(op_ty_results, result)
    !isnothing(indices_are_sorted) && push!(attributes, namedattribute("indices_are_sorted", Attribute(indices_are_sorted)))
...
```
"""
function recv(token::Value; result::Tuple{Vararg{IR.Type}}, channel_handle::Attribute, is_host_transfer::Union{Bool, Nothing}=nothing, location=Location())
    op_ty_results = IR.Type[result..., ]
    operands = Value[token, ]
    owned_regions = Region[]
    successors = Block[]
...
scatters the split parts between the processes to produce the `result`.
 ...
 channel_handle = #stablehlo.channel_handle<handle = 0, type = 0>
    ...
    ```
"""
function reduce_scatter(operand::Value; result::IR.Type, scatter_dimension::Int64, replica_groups::Attribute, channel_handle::Union{Attribute, Nothing}=nothing, use_global_device_ids::Union{Attribute, Nothing}=nothing, computation::Region, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[operand, ]
    owned_regions = Region[computation, ]
    successors = Block[]
...
https://github.com/openxla/stablehlo/blob/main/docs/spec.md#scatter
 ...
 unique_indices = false
   ...
   ```
"""
function scatter(inputs::Vector{Value}, scatter_indices::Value, updates::Vector{Value}; result::Tuple{Vararg{IR.Type}}, scatter_dimension_numbers::Attribute, indices_are_sorted::Union{Bool, Nothing}=nothing, unique_indices::Union{Bool, Nothing}=nothing, update_computation::Region, location=Location())
    op_ty_results = IR.Type[result..., ]
...
```
"""
function send(inputs::Vector{Value}, token::Value; result::Union{Nothing, IR.Type}=nothing, channel_handle::Attribute, is_host_transfer::Union{Bool, Nothing}=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[inputs..., token, ]
    owned_regions = Region[]
    successors = Block[]
