...
function collective_permute(
    operand::Value;
    result::Union{Nothing,IR.Type}=nothing,
    source_target_pairs::IR.DenseElements{Int64},
    channel_handle::Union{ChannelHandle,Nothing}=nothing,
    location::Location=Location(),
)
    op_ty_results = IR.Type[]
...
function custom_call(
    ...
    called_computations::Union{Vector{IR.FlatSymbol},Nothing}=nothing,
    operand_layouts::Union{Vector{IR.DenseElements{Int64}},Nothing}=nothing,
    result_layouts::Union{Vector{IR.DenseElements{Int64}},Nothing}=nothing,
    output_operand_aliases::Union{Vector{OutputOperandAlias},Nothing}=nothing,
    location::Location=Location(),
)
    op_ty_results = IR.Type[result...,]
    operands = Value[inputs...,]
