...
for op in (:rfft, :fft, :ifft)
    ...
    @eval AbstractFFTs.$(plan_f)(x::Reactant.TracedRArray{T}, dims = 1:ndims(x)) where {T} = $(plan_name){T, typeof(dims)}(dims)
    @eval Base.:*(p::$(plan_name){T}, x::Reactant.TracedRArray{T}) where {T} =
        AbstractFFTs.$(op)(x, p.dims)
...
for op in (:irfft,)
    ...
    @eval AbstractFFTs.fftdims(p::$(plan_name)) = p.dims
    @eval AbstractFFTs.$(plan_f)(x::Reactant.TracedRArray{T}, d::Integer, dims = 1:ndims(x)) where {T} = $(plan_name){T, typeof(dims), typeof(d)}(dims, d)
    @eval Base.:*(p::$(plan_name){T}, x::Reactant.TracedRArray{T}) where {T} =
        AbstractFFTs.$(op)(x, p.length, p.dims)
