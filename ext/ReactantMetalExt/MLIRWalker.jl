...
mutable struct TranslationContext
    ...
    # Constant value tracking: IR.Value → (ir_shape, flat_values_in_ir_order)
    # Used for compile-time evaluation of ops like scatter when all inputs are constants
    const_ir_data::Dict{Any,Tuple{Vector{Int},Vector{Float64}}}
end
...
function TranslationContext()
    return TranslationContext(
        ...
        Metal.MPSGraphs.MPSGraphTensor[],
        Any[],
        Dict{Any,Tuple{Vector{Int},Vector{Float64}}}(),
    )
end
...
function handle_constant(ctx, op)
    ...
    if isempty(out_shape)
        ...
        # Track scalar constant
        ctx.const_ir_data[IR.result(op, 1)] = (out_shape, [value])
    elseif IR.issplat(val_attr)
        ...
        # Track splat constant (expand to flat array)
        total = max(1, prod(out_shape))
        ctx.const_ir_data[IR.result(op, 1)] = (out_shape, fill(value, total))
    else
        ...
        if length(nums) == total
            ...
            result = ph
            # Track multi-value constant (nums is already in IR row-major order)
            ctx.const_ir_data[IR.result(op, 1)] = (out_shape, nums)
        else
...
function handle_scatter(ctx, op)
    # Try constant-folding: if all operands are compile-time constants,
    # evaluate the scatter in Julia and create the result as a constant.
    # This avoids MPSGraph's scatterND which may behave differently across macOS versions.
    op1_val = IR.operand(op, 1)
    ...
    if all_const
        ...
        # Evaluate scatter on CPU: for each row of indices, write update value
        # into the base tensor at the specified position.
        # This handles the standard scatter pattern with:
        #   inserted_window_dims = all dims, scatter_dims_to_operand_dims = [0,...,n-1]
        result_flat = copy(base_flat)
        n_updates = idx_shape[1]      # number of scatter updates (rows)
        idx_depth = length(idx_shape) >= 2 ? idx_shape[2] : 1  # index vector length

        # Determine the update value: check the scatter body.
        # If the body returns a constant (common in Enzyme Forward mode), detect it.
        scatter_region = IR.region(op, 1)
        scatter_block = first(scatter_region)
        body_ops = collect(scatter_block)
        use_body_const = false
        body_const_val = 0.0
        # Check if the body is a single return of a constant
        for body_op in body_ops
            bname = IR.name(body_op)
        ...
        for i in 0:(n_updates - 1)
            ...
            flat_pos = 0
            stride = 1
            for d in (length(out_shape)):-1:1
                flat_pos += nd_idx[d] * stride
                stride *= out_shape[d]
            end
            # Write the update value
            update_val = use_body_const ? body_const_val : upd_flat[i + 1]
            result_flat[flat_pos + 1] = update_val
        end

        # Create the result as a constant placeholder
        mps_dtype = julia_to_mps_dtype(out_dtype)
        flat_vals = convert(Vector{mps_dtype}, result_flat)
        julia_shape = length(out_shape) >= 2 ? reverse(out_shape) : out_shape
        julia_arr = reshape(flat_vals, julia_shape...)
        mtl = MtlArray(julia_arr)
        ph = Metal.MPSGraphs.placeholderTensor(
            ctx.graph, julia_shape, mps_dtype, "scatter_const_$(ctx.op_count)"
        )
        push!(ctx.const_placeholders, ph)
        push!(ctx.const_mtl_values, mtl)
        # Track the result as a constant too (for downstream ops)
        ctx.const_ir_data[IR.result(op, 1)] = (out_shape, result_flat)
        return ctx.value_map[IR.result(op, 1)] = ph
    end

    # Non-constant path: use MPSGraph scatterND
    operand = ctx.value_map[op1_val]   # base tensor
    indices = ctx.value_map[op2_val]   # scatter indices (ND)
    updates = ctx.value_map[op3_val]   # update values
...
The caller (PJRTPlugin._loaded_exec_execute) wraps them in MTLBuffer handles.
