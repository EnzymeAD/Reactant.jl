# Turing.jl model for GP Classification
# f = chol(alpha^2 * K_base) * f_tilde, y ~ Bernoulli(logistic(f))

using Turing, Distributions, LinearAlgebra

@model function turing_gp_classify(K_base, Y, n)
    ...
    Turing.@addlogprob! sum(logpdf.(BernoulliLogit.(f), Y))
end
...
function setup(data)
    ...
    model = turing_gp_classify(K_base, Y, n)
    return (turing_model = model, model_name = "GP Classify")
end
