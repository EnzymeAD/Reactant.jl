...
function train_step(
    ...
    (; val, derivs) = Enzyme.gradient(
        ...
        Const(p_z),
        Const(Gz), Const(Δt),
        Const(target_r), Const(target_i), Const(invN_val),
        Const(Val(N_DT))
    )
...
function setup_bloch_benchmark(Nspins::Int)
    ...
    cpu = (; p_z=z_cpu, Gz=Gz_tl, Δt=Δt_tl, B1_r=B1_r_tl, B1_i=B1_i_tl,
             target_r, target_i, invN, N_Δt_val)

    # RArrays for Reactant benchmark
    ra = (; p_z=Reactant.to_rarray(z_cpu),
...
function run_bloch_rf_optimization_benchmark!(results, backend)
    ...
    for Nspins in spin_counts
        ...
        if backend == "CPU"
            ...
            train_step(
                ctx.cpu.B1_r, ctx.cpu.B1_i,
                ctx.cpu.p_z, ctx.cpu.Gz, ctx.cpu.Δt,
                ctx.cpu.target_r, ctx.cpu.target_i, ctx.cpu.invN,
                lr, ctx.cpu.N_Δt_val
            )
        ...
        time = Reactant.Profiler.profile_with_xprof(
            train_step,
            ctx.ra.B1_r, ctx.ra.B1_i,
            ctx.ra.p_z, ctx.ra.Gz, ctx.ra.Δt,
            ctx.ra.target_r, ctx.ra.target_i, ctx.ra.invN,
            lr, ctx.ra.N_Δt_val;
            nrepeat=25
        )
