...
function Distributions._rand!(rng::Random.AbstractRNG, d::DiagonalVonMises, x::AbstractVector{<:Float32})
    dv = Distributions.product_distribution(Distributions.VonMises.(Float64.(d.μ), Float64.(d.κ)))
    x64 = rand(rng, dv)
    x .= Float32.(x64)
    return x
end
...
function build_post(fov, npix, dataf, backend)
    ...
    intpr = (
        lg=ArrayPrior(
            IIDSitePrior(ScanSeg(), Normal(T(0.0), T(0.2)));
            LM=IIDSitePrior(ScanSeg(), Normal(T(0.0), T(1.0))),
        ),
        gp=ArrayPrior(
            IIDSitePrior(ScanSeg(), DiagonalVonMises(T(0.0), T(inv(π^2))));
