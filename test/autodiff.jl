...
@testset "Basic Forward Mode" begin
	...
	res1 = @jit(fwd(set_abi(Forward, Reactant.ReactantABI), Duplicated, ConcreteRArray(ones(3,2)), ConcreteRArray(3.1*ones(3,2))))
	...
	@test typeof(ores1) == NamedTuple{(Symbol("1"),Symbol("2")), Tuple{Array{Float64, 2}, Array{Float64, 2}}}

	res1 = @jit(fwd(set_abi(ForwardWithPrimal, Reactant.ReactantABI), Duplicated, ConcreteRArray(ones(3,2)), ConcreteRArray(3.1*ones(3,2))))
	...
	res1 = @jit(fwd(set_abi(Forward, Reactant.ReactantABI), Const, ConcreteRArray(ones(3,2)), ConcreteRArray(3.1*ones(3,2))))
	...
	res1 = @jit(fwd(set_abi(ForwardWithPrimal, Reactant.ReactantABI), Const, ConcreteRArray(ones(3,2)), ConcreteRArray(3.1*ones(3,2))))
