...
@testset "Basic Forward Mode" begin
    ...
    @test typeof(res1) == Tuple{ConcretePJRTArray{Float64,2,1,Sharding.NoShardInfo}}
    @test res1[1] ≈ ores1[1]
    ...
    res1 = @jit(
        fwd(Forward, Const, ConcretePJRTArray(ones(3, 2)), ConcretePJRTArray(3.1 * ones(3, 2)))
    )
...
@testset "Forward Gradient" begin
    ...
    @test typeof(res) == Tuple{
        Enzyme.TupleArray{ConcretePJRTNumber{Float64,1,Sharding.NoShardInfo},(2, 2),4,2}
    }
    @test res[1] ≈ ones(2, 2)
end
...
@testset "Cached Return: Issue #416" begin
    ...
    @test stret.st isa ConcretePJRTArray
    @test stret.st ≈ x .+ 1
    ...
    @test @allowscalar all(isone, ret[1])
    @test stret.st1 isa ConcretePJRTArray
    @test stret.st1 ≈ x .+ 1
    @test stret.st2 isa ConcretePJRTArray
    @test stret.st2 ≈ x .+ 1
