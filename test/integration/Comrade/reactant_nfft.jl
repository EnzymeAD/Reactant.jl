...
struct ReactantNFFTPlan{T,D,K<:AbstractArray, arrTc, vecI, vecII, FP, BP, INV, SM} <: AbstractNFFTPlan{T,D,1} 
    N::NTuple{D,Int}
    NOut::NTuple{1,Int}
    J::Int
    k::K
...
function ReactantNFFTPlan(k::AbstractArray{T}, N::NTuple{D,Int};
    ...
    ReactantNFFTPlan{T, D, typeof(k), typeof(tmpVec), typeof(deconvIdx), typeof(windowLinInterp), typeof(FP), typeof(BP), typeof(winHatInvLUT), typeof(B_)}(N, NOut, J, k, Ñ, dims_, FP, BP, tmpVec, tmpVecHat, 
...
function AbstractNFFTs.convolve!(p::ReactantNFFTPlan{T,D}, g::Reactant.AnyTracedRArray, fHat::Reactant.AnyTracedRArray) where {D,T}
  mul!(fHat, transpose(p.B), vec(g)) 
  return
end

function AbstractNFFTs.convolve_transpose!(p::ReactantNFFTPlan{T,D}, fHat::Reactant.AnyTracedRArray, g::Reactant.AnyTracedRArray) where {D,T}
  mul!(vec(g), p.B, fHat)
  return
end
...
function AbstractNFFTs.deconvolve!(p::ReactantNFFTPlan{T,D}, f::AbstractArray, g::AbstractArray) where {D,T}
...
function LinearAlgebra.mul!(fHat::Reactant.AnyTracedRArray, p::ReactantNFFTPlan{T,D}, f::Reactant.AnyTracedRArray; 
    ...
    t3 = @elapsed @inbounds NFFT.convolve!(p, p.tmpVec, fHat)
end
