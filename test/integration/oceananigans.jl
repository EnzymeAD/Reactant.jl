...
using Oceananigans.TimeSteppers: time_step!, first_time_step!
...
function estimate_tracer_error(model, initial_temperature, initial_salinity, u_wind_stress, v_wind_stress, temp_flux, Δz)
...
function differentiate_tracer_error(model, Tᵢ, Sᵢ, u_wind_stress, v_wind_stress, temp_flux, Δz,
                                   dmodel, dTᵢ, dSᵢ, du_wind_stress, dv_wind_stress, dtemp_flux, dΔz)
...
vgrid          = make_grid(varch, Nx, Ny, Nz, z_faces)
vmodel         = build_model(vgrid, Δt₀, parameters)
vT_flux        = T_flux_init(vmodel.grid, parameters)
vu_wind_stress = u_wind_stress_init(vmodel.grid, parameters)
vv_wind_stress = v_wind_stress_init(vmodel.grid, parameters)
vTᵢ, vSᵢ        = temperature_salinity_init(vmodel.grid, parameters)
vΔz            = Δz
...
# Make the grid:
rgrid          = make_grid(rarch, Nx, Ny, Nz, z_faces)
rmodel         = build_model(rgrid, Δt₀, parameters)
rT_flux        = T_flux_init(rmodel.grid, parameters)
ru_wind_stress = u_wind_stress_init(rmodel.grid, parameters)
rv_wind_stress = v_wind_stress_init(rmodel.grid, parameters)
rTᵢ, rSᵢ        = temperature_salinity_init(rmodel.grid, parameters)
rΔz            = Reactant.ConcreteRArray(Δz)
...
du_wind_stress = Field{Face, Center, Nothing}(rmodel.grid)
dv_wind_stress = Field{Center, Face, Nothing}(rmodel.grid)
dT_flux        = Field{Center, Center, Nothing}(rmodel.grid)
dΔz            = Enzyme.make_zero(rΔz)

# Trying zonal transport:
@testset "Oceananigans Integration" begin
    @info "Compiling the model run..."
    tic = time()
    restimate_tracer_error = @compile raise_first=true raise=true sync=true estimate_tracer_error(rmodel, rTᵢ, rSᵢ, ru_wind_stress, rv_wind_stress, rT_flux, rΔz)
    rdifferentiate_tracer_error = @compile raise_first=true raise=true sync=true  differentiate_tracer_error(rmodel, rTᵢ, rSᵢ, ru_wind_stress, rv_wind_stress, rT_flux, rΔz,
                                                                                                            dmodel, dTᵢ, dSᵢ, du_wind_stress, dv_wind_stress, dT_flux, dΔz)
    ...
    tic = time()
    voutput = estimate_tracer_error(vmodel, vTᵢ, vSᵢ, vu_wind_stress, vv_wind_stress, vT_flux, vΔz)
    vanilla_toc = time() - tic
    @show vanilla_toc

    # Spinup the model for a sufficient amount of time, save the T and S from this state:
    tic = time()
    routput = restimate_tracer_error(rmodel, rTᵢ, rSᵢ, ru_wind_stress, rv_wind_stress, rT_flux, rΔz)
    run_toc = time() - tic
    @show run_toc

    @info "Vanilla output vs. Reactant output:"
    @test routput ≈ voutput atol = 1e-3 rtol = 1e-2


    tic = time()
    dedν = rdifferentiate_tracer_error(rmodel, rTᵢ, rSᵢ, ru_wind_stress, rv_wind_stress, rT_flux, rΔz, dmodel, dTᵢ, dSᵢ, du_wind_stress, dv_wind_stress, dT_flux, dΔz)
    ad_toc = time() - tic
